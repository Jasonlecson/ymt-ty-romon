...
******************************************************************************/
#if 1 //1:国内  0:国外
#define PRODUCT_KEY "39fko7eyn957oqh6"    //开发平台创建产品后生成的16位字符产品唯一标识
#else
#define PRODUCT_KEY "x6pawkkb3zq8q9fn"    //开发平台创建产品后生成的16位字符产品唯一标识
#endif

#define MCU_VER "1.1.5"         //用户的软件版本,用于MCU固件升级,MCU升级版本需修改
...
#define WIFI_UART_RECV_BUF_LMT          500              //串口数据接收缓存区大小,如MCU的RAM不够,可缩小
#define WIFI_DATA_PROCESS_LMT           500              //串口数据处理缓存区大小,根据用户DP数据大小量定,必须大于24
#else
#define WIFI_UART_RECV_BUF_LMT          500             //串口数据接收缓存区大小,如MCU的RAM不够,可缩小

//请在此处选择合适的串口数据处理缓存大小（根据上面MCU固件升级包选择的大小和是否开启天气服务来选择开启多大的缓存）
#define WIFI_DATA_PROCESS_LMT           500             //串口数据处理缓存大小,如需MCU固件升级,若单包大小选择256,则缓存必须大于260,若开启天气服务,则需要更大
...
/******************************************************************************
                      5:MCU是否需要支持校时功能
如需要请开启该宏,并在Protocol.c文件内 mcu_write_rtctime 实现代码
mcu_write_rtctime内部有#err提示,完成函数后请删除该#err
mcu在wifi模块正确联网后可调用 mcu_get_system_time()函数发起校时功能
******************************************************************************/
...
如需要请开启该宏,并且mcu在需要获取格林时间处调用mcu_api.c文件内 mcu_get_green_time
并在protocol.c文件 mcu_get_greentime 函数内查看结果,
mcu_get_greentime内部有#err提示,完成函数后请删除该#err
mcu在wifi模块正确联网后可调用 mcu_get_green_time() 函数发起校时功能
******************************************************************************/
...
//当前温度(只上报)
//备注:
#define DPID_TEMP_CURRENT 6
//相对湿度(只上报)
//备注:
#define DPID_HUMIDITY_CURRENT 7
//故障告警(只上报)
//备注:0无故障 1故障 2肥力过高 4肥力过低 8湿度过高  16湿度过低  32温度过高 64温度过低 128 光照过高  256 光照过低  512 水位过高 1024 光照总量不足
#define DPID_FAULT 10
//模式(可下发可上报)
//备注:0: 自动
//1: 手动
#define DPID_MODE 101
//肥力(只上报)
//备注:
#define DPID_FERTILITY 102
//水位(只上报)
//备注:
#define DPID_WATER 103
//浇水次数(只上报)
//备注:浇水次数，每次上报1，服务器做统计，显示周/月统计曲线
#define DPID_WATER_COUNT 104
//植物类型(可下发可上报)
//备注:0: 圣女果
//1: 千禧
//2: 小米辣
...
//57：芝麻叶
//58：迷迭香
#define DPID_TYPE 105
//光照度(只上报)
//备注:
#define DPID_ILLUMINATION 108
//植物开始日期(只上报)
//备注:更换植物时下发新的时间，用来计算当前植物的浇水总次数、累计培育时间。时间戳秒10位
#define DPID_START_DATE 109
//基质剩余量(可下发可上报)
//备注:设置植物种类时可以更新。
#define DPID_MATRIX 110
//爱心值(只上报)
//备注:固件在app/按键/自动浇水后上报要增加的爱心值，服务器做累计操作。
#define DPID_LOVE_VALUE 111
//浇水次数(可下发可上报)
//备注:浇水次数，显示累计浇水次数，可重置0，存储在设备中
#define DPID_WATER_COUNT_DEVICE 112
//累计收获次数(可下发可上报)
//备注:累计收获次数，设置植物种类时+1,存储在设备
#define DPID_CUMULATIVE_HARVEST 113
//累计培育开始时间(可下发可上报)
//备注:用于计算累计培育天数，长按重置成当前时间，时间戳秒10位
#define DPID_BEGIN_DATE 114
//手动刷新(可下发可上报)
//备注:手动刷新命令，发送true时则立即将水位、温度、湿度、光照、肥力数据上报
#define DPID_MANUAL_REFRESH 115
//夜间报警(可下发可上报)
//备注:晚上8点到早上8点不报警、不自动浇水开关； true开 fale关。 
#define DPID_REPORT_NIGHT 116
//温度安全值(可下发可上报)
//备注:温度安全值 ,号分割，如最小20最大30度，"20,30"
#define DPID_SAFE_TEMPERATURE 117
//湿度安全值(可下发可上报)
//备注:湿度安全值  ,号分割，如最小10最大30湿度： "10,30"
#define DPID_SAFE_HUMIDITY 118
//光照安全值(可下发可上报)
//备注:光照安全值
#define DPID_SAFE_ILLUMINATION 119
//安全肥力(可下发可上报)
//备注:安全肥力
#define DPID_SAFE_FERTILITY 120
//光照不足参数(可下发可上报)
//备注:
#define DPID_LIGHT_LOW_LIMIT 121
//补光灯开启模式(可下发可上报)
//备注:补光灯开关
//0:自动
//1:手动
#define DPID_FILL_LIGHT_MODE 122
//补光灯(可下发可上报)
//备注:补光灯
#define DPID_FILL_LIGHT 123
